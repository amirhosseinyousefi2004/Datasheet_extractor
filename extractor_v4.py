...
FIELD_DEFINITIONS = {
    "Tag_number":["tag number","equipment tag","tag"],
    "Equipment":["equipment","separator","vessel","reactor","tank"],
    "Position":["orientation","equipment orientation","mounting orientation"],
    "MDMT_Design":["mdmt","minimum design metal temperature"],
    "Temperature_Design":["design temperature"],
    "Temperature_Operating":["operating temperature","normal temperature"],
    ...
    "Length":["length","overall length","tan to tan"],
...
class DatasheetExtractor:
    ...
    def __init__(self, model_path="/home/amirhossyousefi/models/embedding_models/bge-large-en-v1.5"):
        self.results = []
    ...
    def process_pdf(self, pdf_path):
        ...
        result = {f: {"value":"", "confidence":0.0, "source":""} for f in FIELDS}
        ...
        field_candidates = {field: [] for field in FIELD_DEFINITIONS}

        for block in blocks:
            emb = self.model.encode(block["text"], convert_to_tensor=True)

            for field, field_emb in self.field_embeddings.items():

                similarity = util.cos_sim(emb, field_emb).max().item()
        ...
        for field, candidates in field_candidates.items():
            if candidates:
                result[field] = max(candidates, key=lambda x: x["confidence"])
    ...
    def _find_neighbors(self, label_block, blocks):
        ...
        for block in blocks:
            ...
            if same_row:
                priority = 0
            elif below_same_column:
                priority = 1
            else:
                continue

            distance = max(dx, 0) + 12 * dy
    ...
    def _validate(self, field, value):
        ...
        if "Pressure" in field:
            return 1.0 if re.search(r'[-+]?\d+(\.\d+)?\s*(bar|barg|psi|psig|kpa|mpa)', value, re.I) else 0.2

        if "Temperature" in field or "MDMT" in field:
            return 1.0 if re.search(r'[-+]?\d+(\.\d+)?\s*(°?c|°?f)', value, re.I) else 0.2

        if field in ["Length", "Internal_Diameter"]:
            return 1.0 if re.search(r'\d+(\.\d+)?\s*(mm|cm|m|in|inch|ft)', value, re.I) else 0.2

        if field == "Position":
            return 1.0 if re.search(r'\b(vertical|horizontal|v|h)\b', value, re.I) else 0.2

        if "Material" in field:
            pats = r'(304|316|ss|carbon steel|duplex|super duplex|alloy|hastelloy|ptfe|pvdf|frp|grp|sa[- ]?\d+)'
            return 1.0 if re.search(pats, value, re.I) else 0.3

        if field == "Tag_number":
            return 1.0 if re.search(r'[A-Z]{1,5}-\d+[A-Z]?', value) else 0.2

        if field == "Equipment":
            return 1.0 if re.search(r'(separator|vessel|tank|reactor|scrubber|filter|coalescer)', value, re.I) else 0.3
    ...
    def export_excel(self, path):
        ...
        for row in self.results:
            ws.append([row.get(f, {}).get("value", "") for f in FIELDS])
